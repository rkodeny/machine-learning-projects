"""
Anomaly Detection in Transaction - Utility Function
...
def score_ocsvm(X, contamination, random_state=42, fit_sample=5000):
    """
    ...
    idx = rng.choice(len(X), size=min(fit_sample, len(X)), replace=False)
...
def run_all_models(X, y, contamination):
    ...
    for name, fn in [
        ...
        ("Local Outlier Forest", lambda: score_lof(X, contamination)),
...
def plot_pr_curves(scores, y, ax=None):
    """Plot precision-recall curve for each model's anomaly score."""
    if ax is None:
